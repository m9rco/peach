...
var Base = Class.extend({
  ...
  construct: function () {
    ...
    this._ = _
    this.sourceRoot(path.join(Util.getpeachPath(), 'tmp'));
    // 用于缓存模板JSON列表以快速查询
  ...
  writeGitKeepFile: function (dirname) {
    ...
    fs.writeFileSync(path.join(dirname, '.gitkeep'), '这只是个占位文件', 'utf8');
  },
  ...
  setDefaultTmp: function () {
    var tmpPath = path.join(Util.getpeachPath(), 'tmp', 'templates');
