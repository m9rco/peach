...
var Widget = Base.extend({
  ...
  init: function () {
    ...
    console.log('  need help? go and open issue: https://github.com/o2team/peach/issues/new');
  },
  ...
  renameContent: function(widgetName, widgetOriginName){
    ...
    //图片重命名
    fs.readdir(path.join(modulePath, 'widget', widgetName, 'images'), function(err, files) {
    files.forEach(function(filename) {
        var oldPath = path.join(modulePath, 'widget', widgetName, 'images', filename),
            newPath = path.join(modulePath, 'widget', widgetName, 'images', filename.replace(pattern, widgetName))
        fs.rename(oldPath, newPath, function(err) {
            if (err) {
              console.log(chalk.red('  修改图片名 ' + filename + ' 失败，请联系组件作者查看图片名是否符合规范。'));
            }
        })
    });
});
