/**
* @fileoverview 解析组件的SCSS引用链，区别于peach_sass_graph
* @author manjiz
*/
		...
		if(Util.existsSync(scssFile.path)) {
		  ...
		  var imports = Util.sassImports(scssFileCont);
			...
			imports.forEach(function(e) {
			  ...
			  var staticPath = path.join('static', '@(scss|sass|css)', scssFile.dir,  pathObj.dir, '?(_)' + pathObj.base + '.+(scss|sass)');
			  var find = glob.sync(path.join('../gb', staticPath), {});
			  if(find.length===0) {
			  	find = glob.sync(staticPath, {});
			  }
			  if(find[0]) {
