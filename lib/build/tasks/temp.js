...
module.exports = function ($, appConf, moduleConf, args) {
  return function (mod, modulePath, appPath) {
    return new Promise(function (resolve, reject) {
      ...
      var argsDomain = (args && args.domain) ? args.domain : false;
      var argsFdPath = (args && args.fdPath) ? args.fdPath : false;
      ...
      var preloadPage = moduleConf.support.preloadPage || { enable: false }
      var needPreload = !!preloadPage.enable
      var failover = moduleConf.support.failover || { enable: false }
      var failoverDomain = appConf.failoverDomain
      var needFailover = !!failover.enable
      var failoverPages = failover.pages || true
      /**
       * 生成.temp中站点地图索引
       */
      function generateTemp () {
        ...
        if (remoteName === 'local') {
          ...
          url = 'http://' + (argsDomain || deployOptions.domain) + (argsFdPath || deployOptions.fdPath) + appConf.app
        }
          ...
          .on('end', function () {
            ...
            if (remoteName !== 'local' && remoteName !== 'preview') {
              ...
              vfs.src(path.join(modulePath, 'dist', 'output', '*.html'))
                .pipe($.if(!!shtmlConf.use, peachMate.combo({
                  app: moduleConf.app,
                  module: moduleConf.module,
                  cwd: appPath,
                  needCombo: !!shtmlConf.needCombo,
                  fdPath: (argsFdPath || deployOptions.fdPath),
                  domain: (argsDomain || deployOptions.domain),
