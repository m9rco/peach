        ...
        if (/\.scss|\.sass/.test(args.fPath)) {
            ...
            if (moduleName !== appConf.common) {
              ...
              for (var i in appSassCacheJSon) {
                if (changedModules.indexOf(i) >= 0) {
                  fs.writeFileSync(path.join(cacheRootPath, 'sass', i, 'cache.json'), JSON.stringify(appSassCacheJSon[i], null, 2));
                }
              }
            }
          }
          ...
          for (var m in generateCssFiles) {
            ...
            streamArr.push(
              vfs.src(generateCssFiles[m])
                .pipe(peachMate.sassGraph({
                  cwd: appPath,
                  app: appConf.app,
                  module: m,
                  map: 'sass_graph.json'
                }))
                .pipe(peachMate.sassFilter({
                  cwd: appPath,
                  app: appConf.app,
                  module: m,
          ...
          es.merge(streamArr).on('end', function () {
            ...
            for (var mod in generateCssFiles) {
              var sassCacheFolder = path.join(cacheRootPath, 'sass', mod);
