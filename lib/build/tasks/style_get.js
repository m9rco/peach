    ...
    return new Promise(function (resolve, reject) {
      ...
      return vfs.src(path.join(modulePath, 'dist', '_static', 'widget', '**', '*_tpl.js'))
        .pipe(through2.obj(function (file, enc, cb) {
          ...
          if (tplOutConf.outJS) {
            var jsContent = '';
            var jsFile = path.join(dirname, filename).replace('_tpl', '');
            jsContent = String(fs.readFileSync(jsFile));
            if (_.isArray(dependency) && dependency.length > 0) {
              dependency.forEach(function (widget) {
                if (commonWidgets.indexOf(widget.widgetName) < 0) {
                  var widgetJS = path.join(appPath, widget.module, 'dist', '_static', 'widget', widget.widgetName, widget.widgetName + '.js');
                  jsContent += String(fs.readFileSync(widgetJS));
                }
              });
            }
            jsContent = uglify.minify(jsContent, {
              fromString: true
            }).code;
            md5 = Util.checksum(new Buffer(content + cssContent + jsContent), 16);
            contentJson.script = jsContent;
          }
