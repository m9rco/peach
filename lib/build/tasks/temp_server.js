/**
* @fileoverview server模式，做地址替换，同时将文件拷贝到.temp目录下
* @author  liweitao
*/
        ...
        vfs.src(path.join(modulePath, 'dist', 'output', '**'), { base: path.join(modulePath, 'dist', 'output') })
          ...
          .pipe(through2.obj(function (file, encoding, cb) {
            ...
            if (file.isBuffer()) {
              ...
              var innerPath = fpath.replace(outputPath, '');
              var innerPathArr = innerPath.split(path.sep);
              innerPathArr.shift();
              file.path = path.join(outputPath, innerPathArr.join(path.sep));
              this.push(file);
              cb();
            }
          }))
