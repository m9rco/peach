...
module.exports = function($, appConf, moduleConf, args) {
  return function(mod, modulePath, appPath) {
    return new Promise(function (resolve, reject) {
      ...
      var cacheFolder = path.join(Util.getpeachPath(), 'cache');
      ...
      var compareFileContent = function(){
        //版本一致，再进行项目内map.json与(线上/catch)的map.json 文件内容对比
        var catchMapMd5Hash = Util.checksum(JSON.stringify(Util.readJsonFile(catchMapJsonGbFile).rev), 16);
        var localMapMd5Hash = Util.checksum(JSON.stringify(Util.readJsonFile(path.join(modulePath, 'dist', 'map.json')).rev), 16);
