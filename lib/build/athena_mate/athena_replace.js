...
function replace(opts) {
  ...
  if (!config.serve) {
    try {
      gMapJson = JSON.parse(fs.readFileSync(path.join(Util.getpeachPath(), 'cache', 'common', config.appConf.app + '_' + config.appConf.common + '.json')).toString());
    } catch (e) {
      gMapJson = null;
    }
  }
    ...
    for (var key in deployObj) {
      if (key === 'local') {
        ...
        if (config.needFailover) {
          resourcePrefix['failover'] = '//' + Util.urlJoin(config.failoverDomain, config.fdPath || deployObj[key].fdPath, config.appConf.app, '/')
        }
  ...
  var stream = through2.obj(function (file, encoding, callback) {
    ...
    if (file.isBuffer()) {
      ...
      if (config.needFailover) {
        ...
        if (canFailover) {
          failoverFile = new gutil.File({
            path: path.join(path.dirname(file.path), 'failover_' + basename),
            cwd: file.cwd,
            base: file.base
          })
        }
      ...
      if (Util.regexps.js.test(extname)) {
        ...
        if (failoverFile) {
          failoverFile.contents = new Buffer(processCss(basename, file.contents.toString(), resourcePrefix['failover'], config, includeJson))
        }
...
function trackResource(filename, value, index, config) {
  ...
  let source = "_static";
  if(filename&&filename.indexOf("allinone")>=0){
    source="output";
  }
  var gDistPath = path.join(config.cwd, config.appConf.common, 'dist', source);
