...
```
$ [sudo] npm install -g peach-html
```

由于国外源实在太慢，建议使用国内源来安装

```
$ [sudo] npm i -g peach-html --registry=http://registry.npm.taobao.org --disturl=http://npm.taobao.org/mirrors/node
```
...
基于命令 ``peach``，同时提供了简写``pea``

### 初始化

首先需要初始化peach，在这一步会有初始化工作目录、输入用户名等操作
...
或者使用简写 ``pea a [项目名称]``,``pea app -h``可以看到该命令的使用方式

同时提供了通过携带参数快速创建项目的命令

```
$ pea a --name lo --description 测试 --sass --template 默认模板
...
或者使用简写 ``pea m [模块名]``,``pea module -h``可以看到该命令的使用方式
...
```
$ pea m --name hhh --description 测试 --sass
```
...
或者使用简写 ``pea pa [页面名]``,``pea page -h``可以看到该命令的使用方式

同时提供了通过携带参数快速创建页面的命令

```
$ pea pa --name hello --description 测试 --sass --remote jd
```
...
然后根据提示一步一步来，将会自动生成peach的页面目录和模板
...
或者使用简写 ``pea w [组件名]``,``pea widget -h``可以看到该命令的使用方式

同时提供了通过携带参数快速创建组件的命令

```
$ pea w --name topbar --sass --description 测试
```
...
然后根据提示一步一步来，将会自动生成peach的widget目录和模板

### 删除命令

通过命令 ``peach delete`` 可以进行 **项目**、**模块**、**页面**、**组件** 的删除，请遵循以下场景使用
...
$ pea del -a [项目名] -m [模块名] -p [页面名称]

// 删除当前目录下项目的某一模块中的某一组件
$ pea del -a [项目名] -m [模块名] -w [组件名称]
```
...
// 删除当前项目的某一模块中的某一页面
$ pea del -m [模块名] -p [页面名称]

// 删除当前项目的某一模块中的某一组件
$ pea del -m [模块名] -w [组件名称]
```
...
在执行``peach page [pageName]``命令生成页面后，可以发现在模块的``page``目录下多了一个以刚刚输入的页面名称``pageName``作为名字的目录，这个目录下面包含 **html/js/css** 三个文件。在``html``文件中一般通过加载各个``widget``的方式来进行开发，具体代码如下：
...
module.exports = {
  ...
  useInclude: { // 启用生成html页面片功能
    ...
    ppeaPrefix: 'include', //页面片路径前缀 include/header.html
    files: { // 需要生成的页面片文件
  ...
  deploy: {  // 需要发布时的配置
    ...
    preview: { // 预览机的配置，名字不能修改，配置内容可以随自己需求修改
      ...
      remotePpea: '/labs.qiang.it/h5/qwd' // 上传到的目录
    },
    jdcfinder: { // jdcfinder
      mode: 'http',
      host: 'jdcfinder',
    ...
    jdTest: {
      ...
      remotePpea: '/export/paipai/resource/static/fd/h5/hellokity', // 上传代码的目录
...
在编写完页面后可以通过``peach build``命令来执行对整个项目的编译，编译后的结果生成在各个模块的``dist``目录下。
...
通过``peach serve``命令可以实时预览正在编辑的页面。你可以在根目录执行这个命令，也可以进入到具体模块目录下去执行这个命令。

``serve`` 可简写成 ``s``。

如果在项目根目录下，可以通过携带参数来决定要浏览的页面：
```
$ peach serve --module [模块名] --page [页面名]
```

如果在模块目录下，可以通过携带参数来决定要浏览的页面：
```
$ peach serve --page [页面名]
```
命令简写

```
$ pea s -m [模块名] --page [页面名]
```
...
``peach publish`` 会将模块重新编译后发布到 ``app-conf.js`` 中配置的机器（包括预览机和开发机）上，同时会将压缩并重命名后的css文件和动态生成的页面片文件发布到机器的对应目录下（预览机器除外）。而在每次执行 ``peach publish`` 后页面片中的时间戳将会自动更新。目前配置了发布到腾讯和京东域的开发机，机器代号分别是 *tencent*
*jdTest*，同时可发布到预览机 *labs.qiang.it* 上。在 ``peach publish`` 的过程中，你可以自行选择需要发布到开发机上的页面和对应静态资源。
...
若发现上传文件一直错误，请使用 `$ pea clear --publish` 命令来清除文件发布的缓存以解决问题（请详见`pea clear` 命令的使用），并提出issue描述问题。

### peach clone

进入到某一模块下，通过``peach clone`` 命令可以复制另一个模块的**widget**到当前模块。

```
$ peach clone [组件名字] --from [来源模块] --to [目标模块，若是当前模块可省略]
```

使用``pea clone -h`` 查看帮助。

### peach widget-publish
...
```
$ peach widget-publish [组件名字]
$ peach widget-publish --widget [组件名字]
```
简写
...
```
$ peach widget-load [组件id] --alias [组件重命名]
$ peach widget-load --id [组件id] --alias [组件重命名]
```
简写

```
$ pea wl [组件id] -a [组件重命名]
$ pea wl -i [组件id] -a [组件重命名]
```
...
```
$ peach map [--module gb] --widget tab
```
...
$ pea clear -p
// 若要删除当前项目某一模块的发布缓存
$ peach clear --module xxx --publish
// 简写
$ pea clear -m xxx -p
```
...
将检测当前peach版本是否最新，若是旧版本将自动安装最新版本peach
...
```
配置地址：/Users/luckyadam/project/jdc/peach-html/.config.json
user_name=luckyadam
...
```
设置地址：/Users/luckyadam/project/jdc/peach-html/.setting.json
report_url=http://aotu.jd.com/peach
```
...
若发现sass文件编译一直错误，请使用 `$ pea clear --sass` 命令来清除sass编译的缓存以解决问题（请详见`pea clear` 命令的使用），并提出issue描述问题。
